...
pub fn capture_screen() -> Result<String, String> {
    ...
    let temp_path = std::env::temp_dir().join(format!("crackit_full_{}.png", timestamp));
    
    // Use macOS native screen capture
    let mut cmd = std::process::Command::new("screencapture");
    // -x: do not play sounds
    // -m: only main monitor
    cmd.arg("-x").arg("-m").arg(&temp_path);
...
pub fn capture_screen_interactive() -> Result<String, String> {
    let timestamp = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos();
    let temp_path = std::env::temp_dir().join(format!("crackit_snip_{}.png", timestamp));
